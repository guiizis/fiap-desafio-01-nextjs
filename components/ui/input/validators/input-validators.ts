...
function requiredRule(value: string) {
  return value.trim().length === 0 ? 'Campo obrigatório.' : null;
}
...
const validationRules: Record<InputValidationKind, ValidationRule> = {
  ...
  email: {
    ...
    validate: (value) => {
      ...
      return EMAIL_REGEX.test(value.trim()) ? null : 'Dado incorreto. Revise e digite novamente.';
    },
  },
  password: {
    ...
    validate: (value) => {
      ...
      return value.trim().length >= 6 ? null : 'A senha deve ter no mínimo 6 caracteres.';
    },
  },
};

export function getValidationKindFromInputType(type?: HTMLInputTypeAttribute): InputValidationKind {
  if (type === 'email') {
    return 'email';
  }
