...
describe('input validators', () => {
  ...
  it('validates email values with expected messages', () => {
    expect(validateInputValue('email', '')).toBe('Campo obrigatório.');
  ...
  it('validates name and password values', () => {
    expect(validateInputValue('name', '')).toBe('Campo obrigatório.');
    expect(validateInputValue('name', 'ab')).toBe('Digite seu nome completo.');
    expect(validateInputValue('name', 'Maria Silva')).toBeNull();

    expect(validateInputValue('password', '')).toBe('Campo obrigatório.');
    expect(validateInputValue('password', '123')).toBe('A senha deve ter no mínimo 6 caracteres.');
